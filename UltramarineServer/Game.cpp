...
Tank::Tank(double x, double y, double angle, Player* player, TankController* Controller, LPSTR NickName):
	PointUnit(x, y, angle),
...
void Tank::SteepProc()
{
	...
	if (Controller->KeyFirePressed && FireReady >= 150)
	{
	...
	if (Controller->KeyPlanePressed && player->PlaneReady == 1)
	{
		new Plane(Controller->Mouse.x, Controller->Mouse.y);
		player->PlaneReady = false;
	}
	double TargetAngle;
	TargetAngle = AngleToPoint(x, y, Controller->Mouse.x, Controller->Mouse.y);
	...
	BYTE Mov = 0;
	if (Controller->KeyForwardPressed) Mov |= 0b10000000;
	if (Controller->KeyLeftPressed)    Mov |= 0b01000000;
	if (Controller->KeyRightPressed)   Mov |= 0b00100000;
	if (Controller->KeyBackPressed)    Mov |= 0b00010000;
...
{
	Serialize(Tank.x, Tank.y, Tank.angle, Tank.TowerAngle, Tank.HitPoints, Tank.NickName, Tank.FireReady, Tank.player->Client->ID);
	//if (CopyingToArray) Serialize(Tank.player->Client->ID);
}
...
Player::Player(LPSTR NickName, TankController* Controller, double x, double y, double angle):
	NickName(NickName),
	Controller(Controller),
	PlaneReady(false),
...
void Player::Spawn()
{
	...
	tank = new Tank(SpawnPoint.x, SpawnPoint.y, SpawnPoint.angle, this, Controller, NickName);
	SpawnProc();
}
...
void GameInit()
{
	...
	//Bot2->Spawn();
	//Bot3->Spawn();
	Server = new GameServer(60004);

	RegisterSendTypes<Tank, Bullet>(ReservedCount);
	//RecvUnit<TankController>::Register(ReservedCount);
	Server->RegisterRecvProc(ReservedCount, true, [](Server::MessageInfo* Message)
	{
		if (Message->Client->Player == NULL) return false;
		UINT ID;
		Message->Array->Deserialize(ID, *(Message->Client->Player->Controller));
		return true;
	});
	...
	Server->RegisterRecvProc(ConnectRequest, false, [](Server::MessageInfo* Message)
	{
		...
		Player* NewPlayer = new Player(NickName, new TankController(), 0, 0, 0);
		NewPlayer->Client = Message->Client;
		NewPlayer->Client->Player = NewPlayer;
		NewPlayer->Spawn();
		return true;
	});
	/*function<bool(MessageInfo* Message)>* RecvConnectProc = new function<bool(MessageInfo* Message)>([&](MessageInfo* Message)
	{
		for (int i = 0; i < ClientsCount; i++)
		{
			if (Clients[i]->Addr.sin_addr.S_un.S_addr == Message->FromAddr.sin_addr.S_un.S_addr) return;
		}
		Clients[ClientsCount] = new Client(Message->FromAddr, ClientsCount);
		SendToClient(Nec, ConnectSuccess, ClientsCount);
		ClientsCount++;
	});*/
}
